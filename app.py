...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Potato Blight Classifier",
    page_icon="🥔",
    layout="centered",
)

# ── Constants ─────────────────────────────────────────────────────────────────
MODEL_PATH       = Path(__file__).parent / "model" / "potato_blight_model.keras"
CLASS_STATS_PATH = Path(__file__).parent / "model" / "class_stats.npz"
# Alphabetical: Early_Blight (index 0), Late_Blight (index 1) — raw_prob = P(Late_Blight)
...
def predict(model, pil_image: Image.Image):
    """raw_prob = P(Late_Blight) — Late_Blight is CLASS_NAMES[1] (alphabetically second)."""
...
st.caption(
    "Upload a potato leaf image and the model will classify it as "
    "**Early Blight** or **Late Blight** using a MobileNetV2 transfer-learning model."
)
st.divider()
...
if uploaded is not None:
    ...
    with col2:
        ...
        if rejected:
            ...
            color = "#e67e22" if label == "Early_Blight" else "#e74c3c"
            st.markdown(
                f"""
            ...
            with st.expander("Raw probabilities"):
                late_prob  = raw_prob
                early_prob = 1.0 - raw_prob
                st.write(f"**Late Blight:** {late_prob * 100:.2f}%")
                st.progress(late_prob)
                st.write(f"**Early Blight:** {early_prob * 100:.2f}%")
                st.progress(early_prob)

    st.divider()
    st.caption("This prediction is for educational purposes. Always verify with expert agronomic advice.")

else:
    st.info("👆 Upload a potato leaf image above to get a prediction.", icon="🖼️")
